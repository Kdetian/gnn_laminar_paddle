...
def load_data(file_name):
    ...
    minimum = paddle.min(flow, axis=0)
    maximum = paddle.max(flow, axis=0)
...
def load_dataset(num_files, normalize, do_read=False, dataset_source='./dataset/dataset_toUse.txt'):
    ...
    max_value = paddle.zeros([3, ], dtype='float32')
    min_value = paddle.zeros([3, ], dtype='float32')

    # Domain boundaries
    domain_boundaries_min = paddle.zeros([2, ], dtype='float32')  # [x_min y_min]
    domain_boundaries_max = paddle.zeros([2, ], dtype='float32')  # [x_max y_max]
    ...
    for file_name in file_list:
        ...
        # change max item and min item in tensor which is lower than 0 to 0

        max_value = paddle.max(paddle.stack([max_value, max_uvp], axis=0), axis=0)
        min_value = paddle.min(paddle.stack([min_value, min_uvp], axis=0), axis=0)

        # Domain boundaries
        local_boundaries_min = paddle.to_tensor([paddle.min(nodes[:, 0]), paddle.min(nodes[:, 1])],
                                                dtype='float32')  # minest of x, minest of y
        local_boundaries_max = paddle.to_tensor([paddle.max(nodes[:, 0]), paddle.max(nodes[:, 1])], dtype='float32')

        domain_boundaries_min = paddle.min(
            paddle.stack([paddle.squeeze(local_boundaries_min, axis=1), domain_boundaries_min], axis=0), axis=0)
        domain_boundaries_max = paddle.max(
            paddle.stack([paddle.squeeze(local_boundaries_max, axis=1), domain_boundaries_max], axis=0), axis=0)

    # channel-wise normalization, mapping u/v/p values into [0, 1]
    if normalize:
        range_x = paddle.subtract(domain_boundaries_max[0], domain_boundaries_min[0])
        range_y = paddle.subtract(domain_boundaries_max[1], domain_boundaries_min[1])
        range_flow = paddle.subtract(max_value, min_value)
        # TODO file_list to file_list_temp
        for file_name in file_list:
            # Normalize node feature vectors
            x = paddle.divide(paddle.subtract(nodes_set[file_name][:, 0:1], domain_boundaries_min[0]), range_x)
            y = paddle.divide(paddle.subtract(nodes_set[file_name][:, 1:2], domain_boundaries_min[1]), range_y)
            objet = paddle.reshape(nodes_set[file_name][:, -1], [-1, 1])
            nodes_set[file_name] = paddle.concat([x, y, objet], axis=1)
            flow_set[file_name] = paddle.divide(paddle.subtract(flow_set[file_name], min_value), range_flow)
        logs.info('(u, v, p) value range was [%s, %s]x[%s, %s]x[ %s, %s] !\nNow it is mapped into [0,1]x[0,1]x[0,1] !',
                  min_value[0].numpy(), max_value[0].numpy(), min_value[1].numpy(), max_value[1].numpy(),
                  min_value[2].numpy(), max_value[2].numpy())
...
def prepare_graph_batch(nodes_set, edges_set, flow_set):
    ...
    for i in np.arange(num_graphs - 1) + 1:
        ...
        edges = edges_set[shape] + all_nodes.shape[0]  # Shift the nodes connectivity
        flow = flow_set[shape]
        # edges    = tf.math.add(edges_set[shape], int(tf.shape(all_nodes)[0]))  # Shift the nodes connectivity

        all_nodes = paddle.concat([all_nodes, nodes], axis=0)
        all_edges = paddle.concat([all_edges, edges], axis=0)
        all_flow = paddle.concat([all_flow, flow], axis=0)
